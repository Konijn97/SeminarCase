...
for(i in 1:nrow(TS)){
  ...
  if(TS$ad[i] >= 1){
    ...
    tempsma = sma(tempseries,h=5,holdout=TRUE)
    forecast(tempsma)
    tempforecast = (tempsma$forecast)
    
    #loop through the predicted values and add them to the baseline series
    for(j in 1:addmin){
      baseline[min(i-1+j,nrow(TS))] = tempforecast[j]
    }
...
for(i in 10:nrow(TS)){
  ...
  if(TS$ad[i] == 0 && baseline[i] == 0){
    ...
    #estimate a model and forecast 5 minutes in the future
    tempsma = sma(tempseries,h=2,holdout=TRUE)
    forecast(tempsma)
    tempforecast = (tempsma$forecast)
    inversebaseline[i] = tempforecast[1]
    baseline[i] = tempforecast[1]
    #loop through the predicted values and add them to the baseline series
...
for(i in 1:nrow(TS)){
  ...
  if(TS$ad[i] >= 1){
    ...
    #loop through the predicted values and add them to the baseline series
    for(j in 1:addmin){
      baseline_BE[min(i-1+j,nrow(TS))] = tempforecast[j]
    }
    #print("Forecast done at place ")
    #print(i)
  } 
...
for(i in 10:nrow(TS)){
  ...
  if(TS$ad[i] == 0 && baseline_BE[i] == 0){
    ...
    forecast(tempsma)
    tempforecast = (tempsma$forecast)
    inversebaseline_BE[i] = tempforecast[1]
    baseline_BE[i] = tempforecast[1]
    #loop through the predicted values and add them to the baseline series
